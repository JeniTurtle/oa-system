...
class jouryuapiChajian extends Chajian{
	...
	public function geturlstr($mod, $act, $can=array())
	{
		$url 	= $this->updatekey;
		$url.= '?m='.$mod.'&a='.$act.'';
		$url.= '&host='.$this->rock->jm->base64encode(HOST).'&version='.VERSION.'&randtime='.time().'&ip='.$this->rock->ip.'&jouryukey='.getconfig('jouryukey').'&sysnum='.$this->systemnum.'';
	...
	*/
	public function officesend($fileid, $lx=0)
	{
		...
		if(getconfig('officeyl')=='1'){
			...
			if(!$bo || is_string($bo))return returnerror(''.$bo.'，'.c('jouryu')->helpstr('topdf').'');
