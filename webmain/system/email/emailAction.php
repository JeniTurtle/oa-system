...
'.getconfig('phppath','php').' '.$ljth.'\task.php email';
		$this->rock->createtxt(''.UPDIR.'/cli/jouryuemailrun.bat', $str1);
		...
		echo "return array(<br>'title'	=>'琢瑜OA',<br>'phppath' => 'F:\php\php-5.6.22\php.exe' <font color=#aaaaaa>//加上这个你php.exe的路径</font><br>)";
		echo '</div>';
		echo '2、在您的win服务器上，开始菜单→运行 输入 cmd 回车(管理员身份运行)，输入以下命令(每30分钟运行一次)：<br>';
		echo '<div style="background:#caeccb;padding:5px;border:1px #888888 solid;border-radius:5px;">';
		echo 'schtasks /create /sc DAILY /mo 1 /du "24:00" /ri 30 /sd "2018/03/01" /st "00:00:05"  /tn "琢瑜OA自动接收邮件" /ru System /tr '.$ljth.'\\'.UPDIR.'\cli\jouryuemailrun.bat';
		echo '</div>';
		
		
		$str1 = 'cd '.ROOT_PATH.''.chr(10).'php '.ROOT_PATH.'/task.php email';
		$spath= ''.UPDIR.'/cli/jouryuemailrun.sh';
