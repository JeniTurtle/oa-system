...
class taskClassAction extends Action
{
	...
	public function starttaskAjax()
	{
		...
		if($lx==0){
			...
			if($lx==1){
				$barr = c('jouryuapi')->starttask();
				if($barr['success'])$barr['data'] = '已通过官网服务开启计划任务';
			}
			if($lx==2){
				$barr = c('jouryuapi')->stoptask();
	...
	public function downbatAjax()
	{
		...
		echo '<font color="red">如您有安装琢瑜OA服务端，就不用根据下面来开启计划任务了</font><br><a target="_blank" style="color:blue" href="'.URLY.'view_taskrun.html">查看官网上帮助</a><br>';
...
'.getconfig('phppath','php').' '.$ljth.'\task.php runt,task';
		$this->rock->createtxt(''.UPDIR.'/cli/jouryutaskrun.bat', $str1);
		...
		echo "return array(<br>'title'	=>'琢瑜OA',<br>'phppath' => 'F:\php\php-5.6.22\php.exe' <font color=#aaaaaa>//加上这个</font><br>)";
		echo '</div>';
		echo '2、在您的win服务器上，开始菜单→运行 输入 cmd 回车(管理员身份运行)，输入以下命令(每5分钟运行一次)：<br>';
		echo '<div style="background:#caeccb;padding:5px;border:1px #888888 solid;border-radius:5px;">';
		echo 'schtasks /create /sc DAILY /mo 1 /du "24:00" /ri 5 /sd "2017/04/01" /st "00:00:05"  /tn "琢瑜OA计划任务" /ru System /tr '.$ljth.'\\'.UPDIR.'\cli\jouryutaskrun.bat';
		echo '</div>';
		

		$str1 = 'cd '.ROOT_PATH.''.chr(10).'php '.ROOT_PATH.'/task.php runt,task';
		$spath= ''.UPDIR.'/cli/jouryutaskrun.sh';
