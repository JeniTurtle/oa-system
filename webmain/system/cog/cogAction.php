...
class cogClassAction extends Action
{
	public function sysinfoAjax()
	{
		...
		$data = array(
			...
			'jouryukey'	=> getconfig('jouryukey'),
			'db_drive'	=> getconfig('db_drive'),
			'version'	=> '琢瑜OAV'.VERSION.'',
			'phpos'	=> PHP_OS,
			'phpver'	=> PHP_VERSION,
	...
	public function getinfoAjax()
	{
		...
		$arr['jouryukey'] 	= getconfig('jouryukey');
		...
		if(getconfig('systype')=='demo')$arr['jouryukey']='';
		$this->returnjson($arr);
	}
	...
	public function savecongAjax()
	{
		...
		$arr['jouryukey'] 	= $this->post('jouryukey');
		...
		$smarr['jouryukey']		= '琢瑜OA官网key，用于在线升级使用';
