...
class upgradeClassAction extends Action
{
	...
	public function dataAjax()
	{
		$jouryu	= c('jouryu');
		$db 	= m('chargems');
		$wet	= $jouryu->getwebsite();
		$barr	= $jouryu->getdata('mode');
	...
	*/
	public function shengjianAjax()
	{
		$id 	= $this->post('id');
		$key 	= $this->post('key');
		$barr	= c('jouryu')->getdata('getinstallfile',array('id'=>$id,'sysnum'=>$this->keyss,'key'=>$key));
	...
	public function shengjianssAjax()
	{
		...
		if($upbo){
			$barr	= c('jouryu')->getdata('getinstallfileid',array('fid'=>$fileid,'sysnum'=>$this->keyss,'key'=>$key));
			if($barr['code'] != 200)exit($barr['msg']);
			$data 	= $barr['data'];
			$type 	= $data['type'];
	...
	private function shengjifile($frs, $key, $modeid)
	{
		...
		$barr	= c('jouryu')->getdata('getfileinstall',array('id'=>$fid,'sysnum'=>$this->keyss,'key'=>$key));
		if($barr['code']!=200)exit($barr['msg']);
	...
	public function tontbudataAjax()
	{
		$lx = (int)$this->get('lx');
		$snum = $this->get('snum');
		$barr	= c('jouryu')->getdata('getaneydata', array('lx'=>$lx,'snum'=>$snum));
		if($barr['code']!=200)exit($barr['msg']);
		$data 	= $barr['data'];
		$msgr 	= '';
	...
	{
		$id = (int)$this->get('id');
		$barr	= c('jouryu')->getdata('getinstallfile',array('id'=>$id,'sysnum'=>$this->keyss));
		if($barr['code']!=200)showreturn('',$barr['msg'],201);
		$data 	= $barr['data'];
		$mrs 	= $data['rs'];
		$rows 	= $data['rows'];
