...
$(document).ready(function(){
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'人事调动('+nowtabs.name+')',
				cont:'人事调动('+nowtabs.name+')的列表的',
				explain:'订阅[人事调动]的列表',
				objtable:a
			});
		},
		getacturl:function(act){
			return js.getajaxurl(act,'mode_hrtransfer|input','flow',{'modeid':modeid});
		},
		...
		daoru:function(){
			window.managelisthrtransfer = a;
			addtabs({num:'daoruhrtransfer',url:'flow,input,daoru,modenum=hrtransfer',icons:'plus',name:'导入人事调动'});
		},
	...
	//表格参数设定
	var bootparams = {
		fanye:true,modenum:modenum,modename:modename,statuschange:false,tablename:jm.base64decode('aHJ0cmFuc2Zlcg::'),
...
//[自定义区域end]
	...
	js.initbtn(c);
	var a = $('#viewhrtransfer_{rand}').bootstable(bootparams);
	c.init();
...
<div id="viewhrtransfer_{rand}"></div>
<!--HTMLend-->