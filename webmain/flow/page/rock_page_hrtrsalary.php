...
$(document).ready(function(){
	{params}
	var modenum = 'hrtrsalary',modename='调薪申请',isflow=1,modeid='37',atype = params.atype,pnum=params.pnum;
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'调薪申请('+nowtabs.name+')',
				cont:'调薪申请('+nowtabs.name+')的列表的',
				explain:'订阅[调薪申请]的列表',
				objtable:a
			});
		},
		getacturl:function(act){
			return js.getajaxurl(act,'mode_hrtrsalary|input','flow',{'modeid':modeid});
		},
		...
		daoru:function(){
			window.managelisthrtrsalary = a;
			addtabs({num:'daoruhrtrsalary',url:'flow,input,daoru,modenum=hrtrsalary',icons:'plus',name:'导入调薪申请'});
		},
	...
	//表格参数设定
	var bootparams = {
		fanye:true,modenum:modenum,modename:modename,statuschange:false,tablename:jm.base64decode('aHJ0cnNhbGFyeQ::'),
		url:c.storeurl(),storeafteraction:'storeaftershow',storebeforeaction:'storebeforeshow',
		params:{atype:atype},
		columns:[{text:"申请人",dataIndex:"base_name",sortable:true},{text:"申请人部门",dataIndex:"base_deptname",sortable:true},{text:"单号",dataIndex:"sericnum"},{text:"职位",dataIndex:"ranking",sortable:true},{text:"申请日期",dataIndex:"applydt"},{text:"调薪幅度",dataIndex:"floats",sortable:true},{text:"生效日期",dataIndex:"effectivedt",sortable:true},{text:"说明",dataIndex:"explain"},{text:"状态",dataIndex:"statustext"},{
...
//[自定义区域end]
	...
	js.initbtn(c);
	var a = $('#viewhrtrsalary_{rand}').bootstable(bootparams);
	c.init();
...
<div id="viewhrtrsalary_{rand}"></div>
<!--HTMLend-->