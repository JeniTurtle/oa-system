...
class publicClassAction extends Action{
	...
	public function fileviewerAction()
	{
		...
		if(!file_exists($str))exit('未安装预览pdf插件，不能预览该文件，可到琢瑜OA官网下查看安装方法，<a target="_blank" href="'.URLY.'view_topdf.html">查看帮助?</a>。');
	...
	*/
	...
	{
		$id = (int)$this->get('id','0');
		return c('jouryuapi')->officesend($id);
	}
	...
	*/
	...
	{
		$id = (int)$this->get('id','0');
		return c('jouryuapi')->officestatus($id);
	}
	...
	*/
	...
	{
		$id = (int)$this->get('id','0');
		return c('jouryuapi')->officedown($id);
	}
}