var MODE	= '',ACTION = '',DIR='',PROJECT='',HOST='',PARAMS='',QOM='jouryu_',apiurl='',token='',device='',CFROM='pc',ISDEMO=false,NOWURL='',nwjsgui=false;
...
js.gethost=function(){
	...
	if(sau.indexOf('demo.jouryu.com')>=0 || sau.indexOf('demo.oaqoa.com')>=0)ISDEMO=true;
...
js.msg = function(lx, txt,sj){
	...
	if(lx=='msg')txt='<font color=red>'+txt+'</font>';var t=72;
...
function appobj1(act, can1){
	var bo = false;
	if(typeof(appjouryu)=='object'){
		if(appjouryu[act]){
			try{appjouryu[act](can1);bo = true;}catch(e){}
		}
	}
	return bo;
}
