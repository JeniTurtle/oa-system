...
function addtabs(d){
	...
	if(num=='home')s='';
	$('#tabslist').append(s);
	$('#view_content').append('<div temp="content" id="content_'+num+'"></div>');
	var urlpms= '';
	...
	$.ajax({
		...
		success: function(da){
			...
			$('#mainloaddiv').remove();
			var s = da;
				s = s.replace(/\{rand\}/gi, js.getrand());
...
function modeltabs(nr){
	$('#tabsmodestese').remove();
	if(nr=='none')return;
	var s='<div id="tabsmodestese" oncontextmenu="return false" style="position:absolute;width:100%;height:100%;filter:Alpha(opacity=20);opacity:0.2;left:0px;top:0px;background-color:#000000;z-index:80;"><div style="margin-top:200px;color:white;text-align:center"><img src="images/mloading.gif" align="absmiddle">&nbsp;加载中...</div></div>';
	$('#view_content').append(s);
}
function changetabs(num){
	if(coloebool)return;
	$("div[temp='content']").hide();
	$("div[temp='tabs']").removeClass();
	$("div[temp='tabs']").addClass('lists');
	var bo = false;
	if(get('content_'+num+'')){
		$('#content_'+num+'').show();
		$('#tabs_'+num+'').addClass('lists active');
...
var im={
	init:function(){
		...
		systemtitle = js.getoption('systemtitle','htmlvi');
		document.title=systemtitle;
		var slogo = js.getoption('systemlogo');
		if(slogo){
			slogo = ''+apiurl+''+slogo+'';
			get('mainlogo').src=''+slogo+'?'+Math.random()+'';
		}
		...
		addtabs({name:'桌面',num:'home',url:'home'});
		...
		nwjs.init();
		nwjs.udpserver(function(msg,info){
			try{im.udpreceive(msg,info);}catch(e){}
		});
	...
	resize:function(){
		var hw=$('.main').height();
		var o2=get('mainnw'),whd=$(window).height();
		if(o2){
			hw=whd+0;
		}else{
			hw = whd-4;if(whd>1000)hw=1000;
			var nt=(whd-hw)*0.5;
			$('.main').css('margin-top',''+nt+'px');
		}
		viewheight = hw-50;
		var o = $("div[resizeh]"),o1,len=o.length,i,mh,nh;
		for(i=0;i<len;i++){
			o1=$(o[i]);
			mh=parseFloat(o1.attr('resizeh'));
			nh=viewheight-mh;
			o1.css('height',''+nh+'px');
		}
	},
	...
	showgroup:function(a){
		...
		for(i=0;i<len;i++){
			d 	= a[i];
			s	= '<div id="group_'+d.id+'"  onclick="im.opengroup('+d.id+')"><img src="'+d.face+'" align="absmiddle">'+d.name+'</div>';
			grouparr[d.id] = d;
			o.append(s);
		}	
	},
	...
	showagent:function(a){
		...
		for(i=0;i<len;i++){
			...
			s+='<td onclick="im.openagent('+d.id+',event);" class="agenttds cursor" width="33.3%"><div class="agenttd">';
			s+='	<span id="agentstotal_'+d.id+'" class="badge">'+s1+'</span>';
			if(d.totals>0)s+='	<font></font>';
			s+='	<div><img width="26" height="26" src="'+d.face+'"></div>';
			s+='	<div class="name">'+d.name+'</div>';
			s+='</div></td>';
			if((i+1)%3==0)s+='</tr><tr>';
		}
		s+='</tr></table>';
		$('#agentlist').html(s);
		$('#agentlist table').click(function(){return false});
		this.changestotl('agentstotal_',1);
	},
	showagents:function(d){
		var s='';
		var s1=d.stotal;if(s1==0)s1='';
		s= '<div onclick="im.openagent('+d.id+',event);return false;" class="lists">';
		s+='<table cellpadding="0" border="0" width="100%"><tr>';
		s+='<td style="padding-right:8px"><img src="'+d.face+'"></td>';
		s+='<td align="left" width="100%"><div class="name">'+d.name+'</div></td>';
		s+='<td align="center"><span id="agentstotal_'+d.id+'" class="badge">'+s1+'</span></td>';
		if(d.totals>0)s+='<td nowrap>&nbsp;<i class="icon-angle-right"></i></td>';
		s+='</tr></table>';
		s+='</div>';
		agentarr[d.id]=d;
		$('#agentlist').append(s);
	...
	openagent:function(id,e){
		...
		addtabs({name:d.name,num:'agent_'+id+'',url:'agent,id='+d.id+'',face:d.face});
	...
	openchat:function(name,type,id,face){
		addtabs({name:name,num:''+type+'_'+id+'',url:'chat,uid='+id+',type='+type+'',face:face,onshow:function(){
	...
	searchssss:function(){
		var o = $('.msousou'),val=strreplace(o.val());
		var d=[];
		if(val==''){
		...
		this.searchright.showAt(off.left,off.top+26,210);
	},
	openurl:function(tit, url){
		var d=450;
		js.tanbody("winiframe",tit,800,d,{html:'<div style="height:'+d+'px"><iframe src="" name="winiframe" width="100%" height="100%" frameborder="0"></iframe></div>',bbar:"none"})
...
var chatclass = function(opts){
	...
	this.cropScreen=function(){
		if(!nwjsgui){
			js.msg('msg','无法使用截屏，请使用客户端,<a href="http://www.htmlvi.com/" target="_blank">[去下载]</a>');
			return;
		}
	...
	this.getemtsbq=function(wj, oi1,oi2, fzd, dx){
		...
		for(i=oi1; i<=oi2; i++){
			oi++;
			s+='<td width="'+j1+'%" title="'+strformat.emotsarr[i+1]+'" align="center"><img onclick="chatobj[\''+this.num+'\'].backemts(\''+strformat.emotsarr[i+1]+'\')" src="images/im/emots/'+wj+'/'+i+'.gif" width="'+dx+'" height="'+dx+'"></td>';
			if(oi % fzd==0)s+='</tr><tr>';
		}
		s+='</tr></table>';
		return s;
...
function agentclass(opts){
	...
	this._init=function(){
		...
		this.menus= menus;
		var s='<table width="100%" style="border-bottom:1px #dddddd solid" cellpadding="0"><tr><td nowrap>&nbsp;&nbsp;</td><td height="39"><div style="height:24px;overflow:hidden;padding-right:5px"><img src="'+this.receinfor.face+'" height="24"></div></td><td id="agenttitle_'+this.rand+'" nowrap>'+this.receinfor.name+'</td><td width="100%"></td>';
	...
	this.regetdata=function(o1,p){
		if(o1)$(o1).html('<img src="images/loadings.gif" height="14" width="15" align="absmiddle"> 加载中...');
		js.ajax('index','getyydata',{'page':p,'event':this.nowevent,'num':this.num},function(ret){
			me.showdata(ret);
		},'none');
	};
	...
	this.showdata=function(a){
		...
		this.setheight();
		if(count>0){
			s = '<div class="showpage" id="showblank_'+this.rand+'">&nbsp;共'+count+'条记录';
